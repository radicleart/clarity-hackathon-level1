...
describe("Deploying contracts", () => {
  it("should deploy nongibles and projects and wait for confirmation", async () => {
    let transaction = await deployContract("projects", new BigNum(0));
    transaction = await deployContract("nongibles", new BigNum(1));
    console.log("=======================================================================================");
  });
});
...
describe("Check contracts deployed", () => {
  ...
  it("should return projects contract address", async () => {
    let args = [];
    let transaction = await callContract(new BigNum(2), "contract-base", "projects", "get-address", args);
    assert.isOk(transaction, "Transaction succeeded");
  })
})
